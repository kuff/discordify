...
module.exports = class Message {
  ...
  async send(message) {
    if (this.obj.author.id === self_id) {
      if (message.embed) {
        this.obj = await this.obj.edit(message);
      } else {
        this.obj = await this.obj.edit(`${this.author}, ${message}`);
      }
    } else {
      if (message.embed) {
  ...
  async sendNew(message, delete_current_message = false) {
    ...
    if (delete_current_message) await this.obj.delete();
    if (message.embed) {
      this.obj = await channel.send(message);
    } else {
      if (this.author.id === self_id) this.obj = await channel.send(message);
      else this.obj = await channel.send(this.author + ", " + message);
    }
  }
