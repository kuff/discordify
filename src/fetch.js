...
module.exports = class Fetch {
  ...
  async get(query, message, args) {
    return new Promise(async resolve => {
      ...
      if (this.queue.length > 0) {
        await message.send("waiting for another process " + "to finish...");
      }
      ...
      process
        .then(async output => {
          if (process === this.queue[this.queue.length - 1]) this.queue = [];
  ...
  async search(query, message, args) {
    ...
    if (this.queue.length > 0) await this.queue[this.queue.length - 1];
    ...
    if (isUri(query)) return await this.getFromUrl(query, message, args);
    ...
    switch (parse(query).hostname) {
      ...
      case "www.youtube.com":
        ...
        if (list_id) {
          ...
          const result = await youtube.getPlaylistById(list_id, message);
          if (result && result.length > 0)
            // if successful, add the playlist id to
            // the video urls for reference when clicked
            result.forEach(elem => (elem.link += `&list=${list_id}`));
          if (!result && id) {
