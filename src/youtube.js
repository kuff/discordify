...
const lib = {
    ...
    Song: class YouTubeSong extends Song {
        ...
        related(history = []) {
            return new Promise((resolve, reject) => {
                ...
                yt.related(this.id, history.length + 3, // make sure to request an excess of videos in case only previously played songs result from the search
                (error, result) => {
            ...
            yt.getById(id, (error, result) => {
                ...
                const output = result.items.map(elem => 
                    ...
                    new lib.Song({
                        ...
                        duration: moment.duration(elem.contentDetails.duration).asSeconds(), // convert video duration data into seconds
                    })
                );
    ...
    search: async (query, message) => 
        new Promise(async resolve => {
            ...
            yt.search(query, 1, { 'type': 'video,playlist' }, // request one response object of type either video or playlist
            async (error, result) => {
