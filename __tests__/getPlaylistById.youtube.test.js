...
it("Returns Undefined If Provided With Invalid Playlist Id", async done => {
  expect(await getPlaylistById("invalid-id", messageMock)).toEqual(undefined);
  done();
});

it("Returns Undefined When Given Playlist With Zero Entries", async done => {
  const list_id = "PLxQTcoJgTJsRHp266JneGvgYQmD3uty0z";
  expect(await getPlaylistById(list_id, messageMock)).toEqual(undefined);
  done();
});
