...
function relatedRaw() {
  return new Promise((resolve, reject) => {
    yt.related(id, memory_size + 5, (error, result) => {
      ...
      return resolve(
        result.items.reduce((cum, current) => {
...
it("Initializes", () => {
  ...
  expect(song).toEqual(input);
});

it("Plays", async done => {
  const song = new YouTubeSong(input);
  const song_stream = await song.play();
  expect(song_stream).toBeInstanceOf(stream.Readable);
...
it("Ignores Non-Matching Ids While Doing Related Search", done => {
  ...
  song
    ...
    .then(result => {
      expect(result.id).not.toEqual(id);
      expect(ids.indexOf(result.id) != -1).toBe(true);
      done();
    });
});
...
it("Does Related Search With One Related Song In Memory", done => {
  ...
  song
    ...
    .then(result => {
      expect(result.id).not.toEqual(id);
      expect(result.id).not.toEqual(ids[0]);
      expect(ids.indexOf(result.id) != -1).toBe(true);
      done();
    });
});
...
it("Does Related Search With Long Memory Of Related Songs", done => {
  ...
  song
    ...
    .then(result => {
      expect(result.id).not.toEqual(id);
      expect(result.id).not.toEqual(ids[0]);
      expect(result.id).not.toEqual(ids[1]);
      expect(result.id).not.toEqual(ids[2]);
      expect(result.id).not.toEqual(ids[3]);
      expect(result.id).not.toEqual(ids[4]);
      expect(ids.indexOf(result.id) != -1).toBe(true);
      done();
    });
});
