const { search, getById, getPlaylistById } = require("../src/youtube.js");
...
it("Fetches Videos", async done => {
  expect(await search(video_query, messageMock)).toEqual(
...
it("Returns Undefined If Search Comes Up Empty", async done => {
  expect(await search(bad_query, messageMock)).toEqual(undefined);
  done();
});
