...
it("Assigns Fields To Reponse Object", () => {
  expect(result).not.toEqual("placeholder string");

  expect(result.link).toEqual(`https://www.youtube.com/watch?v=${id}`);
  expect(result.id).toEqual(raw.items[0].id);
  expect(result.message).toEqual({});
  expect(result.title).toEqual(raw.items[0].snippet.title);
  expect(result.artist).toEqual(raw.items[0].snippet.channelTitle);
  expect(result.thumbnail).toEqual(raw.items[0].snippet.thumbnails.default.url);
  expect(result.plays).toEqual(raw.items[0].statistics.viewCount);
  expect(result.duration).toEqual(
    moment.duration(raw.items[0].contentDetails.duration).asSeconds()
  );
});
...
it("Handles Multiple Ids", async done => {
  ...
  yt.getById(ids, (error, result) => {
    ...
    expect(test.map(elem => elem.id)).toEqual(
      result.items.map(elem => elem.id)
    );
    done();
  });
});
